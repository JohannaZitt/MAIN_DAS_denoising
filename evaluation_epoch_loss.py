...
# Step = müsste doch eigentlich epochen Anzahl sein?!?! ist aber 1000, obwohl auf 2000 epochn trainiert
...
# Todo: mal ADAM Algo ansehen, ob learning rate während training angepasst wird.


experiments = ['01_ablation_horizontal', '02_ablation_vertical', '03_accumulation_vertical', '04_accumulation_horizontal']


output = [['model', 'train_last', 'validation_last', 'train_mean', 'validation_mean', 'train_min', 'validation_min']]
...
for experiment in experiments:
    ...
    validation_min = np.min(validation_loss[:,2])
    validation_mean = np.mean(validation_loss[:,2])
    validation_last = validation_loss[-1,2]

    train_min = np.min(train_loss[:, 2])
    train_mean = np.mean(train_loss[:, 2])
    train_last = train_loss[-1, 2]

    calculated_values = [experiment, train_last, validation_last, train_mean, validation_mean, train_min, validation_min]

    output.append(calculated_values)
