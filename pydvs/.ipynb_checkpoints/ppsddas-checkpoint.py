...
class PPSDDAS(object):
    def __init__(self, stats, multi_channel=False, skip_on_gaps=False,
                 db_bins=(-200, -50, 1.), ppsd_length=3600.0, overlap=0.5,
                 special_handling=None, freq_smoothing_width_octaves=1.0,
                 freq_step_octaves=0.125, freq_limits=None,
                 **kwargs):  # @UnusedVariable

        # save things related to args
        if(multi_channel==True):
            # DCB - disabling channel-specific ID, since we want to (potentially) merge multiple channels together.
            self.id = "DAS.DAS.DAS.DAS"  # All your base is belong to DAS.
        else:
            #self.id = "%(network)s.%(station)s.%(location)s.%(channel)s" % stats
            self.id = "%(network)s.%(station)s.%(location)s.DAS" % stats
        ...
        # make an initial dummy psd and to get the array of freqs
        ...
        if freq_limits is None:
            freq_limits = (self.psd_freqs[0], self.psd_freqs[-1])
        self._setup_freq_binning(
            freq_smoothing_width_octaves, freq_step_octaves, freq_limits)
        # setup db binning
    ...
    @property
    def freq_bin_centers(self):
        """
        Return centers of freq bins (geometric mean of left and right edge of
        freq smoothing ranges).
        """
        return self._freq_binning[2, :]

    @property
    def freq_xedges(self):
        """
        Returns edges of freq histogram bins (one element longer than number
        of bins). These are the edges of the plotted histogram/pcolormesh, but
        not the edges used for smoothing along the freq axis of the psd
        (before binning).
        """
        return np.concatenate([self._freq_binning[1, 0:1],
                               self._freq_binning[3, :]])

    @property
    def freq_bin_left_edges(self):
        """
        Returns left edges of freq bins (same length as number of bins).
        These are the edges used for smoothing along the freq axis of the psd
        (before binning), not the edges of the histogram/pcolormesh in the
        plot.
        """
        return self._freq_binning[0, :]

    @property
    def freq_bin_right_edges(self):
        """
        Returns right edges of freq bins (same length as number of bins).
        These are the edges used for smoothing along the freq axis of the psd
        (before binning), not the edges of the histogram/pcolormesh in the
        plot.
        """
        return self._freq_binning[4, :]
    ...
    def _setup_freq_binning(self, freq_smoothing_width_octaves,
                              freq_step_octaves, freq_limits):
        """
        Set up freq binning.
        """
        # we step through the freq range at step width controlled by
        # freq_step_octaves (default 1/8 octave)
        freq_step_factor = 2 ** freq_step_octaves
        # the width of frequencies we average over for every bin is controlled
        # by freq_smoothing_width_octaves (default one full octave)
        freq_smoothing_width_factor = \
            2 ** freq_smoothing_width_octaves
        # calculate left/right edge and center of first freq bin
        # set first smoothing bin's left edge such that the center frequency is
        # the lower limit specified by the user (or the lowest freq in the
        # psd)
        per_left = (freq_limits[0] /
                    (freq_smoothing_width_factor ** 0.5))
        per_right = per_left * freq_smoothing_width_factor
        per_center = math.sqrt(per_left * per_right)
        # build up lists
        per_octaves_left = [per_left]
        per_octaves_right = [per_right]
        per_octaves_center = [per_center]
        # do this for the whole freq range and append the values to our lists
        while per_center < freq_limits[1]:
            # move left edge of smoothing bin further
            per_left *= freq_step_factor
            # determine right edge of smoothing bin
            per_right = per_left * freq_smoothing_width_factor
            # determine center freq of smoothing/binning
            per_center = math.sqrt(per_left * per_right)
            # append to lists
        ...
        valid = per_octaves_right > self.psd_freqs[0]
        valid &= per_octaves_left < self.psd_freqs[-1]
    ...
    def add(self, stream, verbose=False):
        ...
        if(self.channel != "DAS"):
	    # DCB - Disabling check for matching trace, want to be able to add any channels
	    # (more onus on user to be careful with inputs)
        ...
        for tr in stream:
            ...
            t2 = tr.stats.endtime
            # DCB note, obspy included a hack to cut one sample off. 
            # Instead checking end time with an extra 1/100th of time buffer
            while t1 + self.ppsd_length <= t2+self.ppsd_length/100.0:
                if(self.channel != "DAS"):
    ...
    def __process(self, tr):
        ...
        smoothed_psd = []
        # do this for the whole freq range and append the values to our lists
        for per_left, per_right in zip(self.freq_bin_left_edges,
                                       self.freq_bin_right_edges):
            specs = spec[(per_left <= self.psd_freqs) &
                         (self.psd_freqs <= per_right)]
...
###########################
    ...
    def calculate_histogram(self, starttime=None, endtime=None,
        ...
        num_freq_bins = len(self.freq_bin_centers)
        num_db_bins = len(self.db_bin_centers)

        # initial setup of 2D histogram
        hist_stack = np.zeros((num_freq_bins, num_db_bins), dtype=np.uint64)
        ...
        inds = inds.reshape((used_count, num_freq_bins)).T
        for i, inds_ in enumerate(inds):
            # count how often each bin has been hit for this freq bin,
        ...
        # low to high psd values for every freq column) of the current
        # histogram stack.
    ...
    def get_percentile(self, percentile=50):
        ...
        return (self.freq_bin_centers, percentile_values)

    def get_mode(self):
        """
        Returns freqs and mode psd values (i.e. for each frequency the psd
        value with the highest probability is selected).

        :returns: (freqs, psd mode values)
        """
        hist = self.current_histogram
        if hist is None:
            return None
        mode = self.db_bin_centers[hist.argmax(axis=1)]
        return (self.freq_bin_centers, mode)

    def get_mean(self):
        """
        Returns freqs and mean psd values (i.e. for each frequency the mean
        psd value is selected).

        :returns: (freqs, psd mean values)
        """
        hist = self.current_histogram
        if hist is None:
            return None
        ...
        return (self.freq_bin_centers, mean)
    ...
    def add_npz(self, filename):
        ...
        same settings, then any time freqs that are not yet covered are added
    ...
    def _add_npz(self, filename):
        ...
        for t, psd in zip(_times_processed, _binned_psds):
            t = UTCDateTime(t)
            if self.__check_time_present(t) and self.channel != "DAS":
                duplicates += 1
                continue
    ...
    def extract_psd_values(self, freq):
        """
        Extract PSD values for given freq in seconds.

        Selects the freq bin whose center freq is closest to the specified
        freq. Also returns the minimum, center and maximum freq of the
        selected bin. The respective times of the PSD values can be accessed as
        :attr:`PPSD.times_processed`.

        :type freq: float
        :param freq: Period to extract PSD values for in seconds.
        :rtype: four-tuple of (list, float, float, float)
        :returns: PSD values for requested freq (at times)
        """
        # evaluate which freq bin to extract
        freq_diff = np.abs(self.freq_bin_centers - freq)
        index = np.argmin(freq_diff)
        freq_min = self.freq_bin_left_edges[index]
        freq_max = self.freq_bin_right_edges[index]
        freq_center = self.freq_bin_centers[index]
        psd_values = [psd[index] for psd in self.psd_values]
        return psd_values, freq_min, freq_center, freq_max

    def plot_temporal(self, freq, color=None, legend=True, grid=True,
                      linestyle="-", marker=None, filename=None, show=True,
                      **temporal_restrictions):
        """
        Plot the evolution of PSD value of one (or more) freq bins over time.
        ...
        :type freq: float (or list thereof)
        :param freq: Period of PSD values to plot. The freq bin with the
            central freq that is closest to the specified value is selected.
        ...
        :param color: Color specification understood by :mod:`matplotlib` (or a
            list thereof in case of multiple freqs to plot). ``None`` for
            default colors.
        :type grid: bool
        :param grid: Enable/disable grid in histogram plot.
        :type legend: bool
        ...
        for freq, color in zip(freqs, colors):
            cur_color = color
            # extract psd values for given freq
            psd_values, freq_min, _, freq_max = \
                self.extract_psd_values(freq)
            if mask is not None:
                psd_values = [x for i, x in enumerate(psd_values)
                              if not mask[i]]
            ## if right edge of freq range is less than one second we label
            ## the line in Hertz
            #if freq_max < 1:
            #    label = "{:.2g}-{:.2g} [Hz]".format(
            #        1.0 / freq_max, 1.0 / freq_min)
            #else:
            label = "{:.2g}-{:.2g} [Hz]".format(freq_min, freq_max)

            for i, (times_, psd_values) in enumerate(
                    self._split_lists(times, psd_values)):
                # only label first line plotted for each freq
                if i:
                    label = None
    ...
    def plot(self, filename=None, show_coverage=True, show_histogram=True,
             ...
             show_noise_models=False, grid=True, show=True,
             max_percentage=None, freq_lim=(0.1, 100), show_mode=False,
        ...
        :param max_percentage: Maximum percentage to adjust the colormap. The
            ...
            is ignored.
        :type freq_lim: tuple of 2 floats, optional
        :param freq_lim: Period limits to show in histogram. When setting
        ...
        :param xaxis_frequency: If set to `True`, the x axis will be frequency
            in Hertz as opposed to the default of freq in seconds.
        """
        import matplotlib.pyplot as plt
        ...
        if show_percentiles:
            # for every freq look up the approximate place of the percentiles
            for percentile in percentiles:
                freqs, percentile_values = \
        ...
        if show_noise_models:
            for freqs, noise_model in (get_nhnm(), get_nlnm()):
                if xaxis_frequency:
                    xdata = 1.0 / freqs
                else:
                    xdata = freqs
        ...
        ax.semilogx()
        #if xaxis_frequency:
        #    xlim = map(lambda x: 1.0 / x, freq_lim)
...
def get_nhnm():
    """
    Returns freqs and psd values for the New High Noise Model.
    For information on New High/Low Noise Model see [Peterson1993]_.
    """
    data = np.load(NOISE_MODEL_FILE)
    freqs = data['model_freqs']
